...
const toPullStream = require('./readme');

test('it converts a sync pullable source to a pull-stream', async function (t) {
  t.plan(9);
  ...
  const pullsource = toPullStream(source);
...
test('it converts an async pullable source to pull stream', async function (t) {
  t.plan(9);
  const upwardsExpected = [
    [0, 'function'],
    [1, 'undefined'],
